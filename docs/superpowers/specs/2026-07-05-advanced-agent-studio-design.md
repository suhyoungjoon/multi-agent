...
- **핵심 문제 3가지**: (1) 팀 상태 가시성 대시보드 (2) 멈춘 패널 깨우기(Enter 재전송)/역할 재주입/compact 트리거 (3) wiki 메모리 읽기+쓰기
- **정체 감지(stuck-check) 토글**: 주기적 정체 감지(STUCK_THRESHOLD 알림)는 켜고 끌 수 있음(기본값 켜짐). `/clear` 추정 감지와 컨텍스트 임계치 자동 조치(자동 역할재주입, 자동 compact)는 이 토글과 무관하게 항상 자동 실행 — wake만 사람이 판단해서 누르는 영역이고 나머지 자동화는 그대로 유지
- **온디맨드 상태 체크**: 주기적 폴링이 꺼져 있어도 사용자가 패널별로 "지금 확인" 버튼을 눌러 즉시 stuck 여부를 볼 수 있음
...
프론트 (2~3초 폴링)
  ...
  GET  /api/watchdog/settings    → 정체 감지 토글 현재 상태 조회 { stuck_check_enabled: bool }
  POST /api/watchdog/settings    → 토글 변경 { stuck_check_enabled: bool } — 기본값 true
  POST /api/pane/{idx}/check     → 온디맨드 즉시 상태 확인 (폴링 on/off와 무관하게 항상 가능, capture-pane 1회 실행 후 stuck/idle 여부 반환)
  POST /api/pane/{idx}/wake       → tmux_control.send_enter() 만 전송 — 진짜 멈춤(STUCK_THRESHOLD) 케이스용 가벼운 깨우기. 자동 트리거 없음, 항상 사용자가 직접 눌러야 실행
  POST /api/pane/{idx}/restart   → tmux_control.reinject_role() 직접 호출 — /clear 추정 케이스용 역할 재주입
  POST /api/pane/{idx}/compact   → tmux_control로 /compact 전송 + 완료 대기 + /context 재확인
```

`wake`와 `restart`는 서로 다른 문제를 다룬다: `wake`는 화면이 오래 안 바뀔 때(프로세스는 살아있고 입력만 안 먹었을 가능성) 쓰는 최소 개입이고, `restart`는 `/clear`로 컨텍스트가 날아간 걸 감지했을 때 역할을 다시 주입하는 것. 둘 다 프로세스를 죽이지 않는 비파괴적 액션이다.

`wake`는 주기적 정체 감지 토글이 꺼져 있어도, 사용자가 `/api/pane/{idx}/check`로 직접 상태를 확인한 뒤 필요하다고 판단하면 언제든 누를 수 있다 — 정체 감지 자동화는 어디까지나 "알려주는" 역할이고, 실제로 깨우는 행위는 토글 상태와 무관하게 항상 사람의 판단과 클릭으로만 일어난다.
...
- 패널 kill + Claude 재실행(파괴적 복구) — v1은 Enter 재전송만 지원. 그래도 안 풀리면 사용자가 직접 tmux attach해서 확인. 필요성이 확인되면 v2에서 추가
