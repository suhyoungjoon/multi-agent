...
> - 추적 안 된 파일이 남아있으면 매 Stop마다 무한 반복 발화하는 버그 발견 → HEAD 커밋 SHA + 변경 경로를 상태로 기록해 재발화 방지
...
- **상태 추적**: `HEAD 커밋 SHA + 변경 경로 목록`을 `.git/team-progress-last-state`에 기록해, 이미 보고한 상태 그대로면(추적 안 된 파일이 계속 방치되는 경우 등) 재발화하지 않음. 커밋되거나 새로 바뀌면 다시 발화
...
민준 pane에서 응답 완료 (Stop 이벤트)
  ...
  → docs/architecture.md가 이번 응답에서 바뀜 + (HEAD, 변경경로) 상태가 이전 기록과 다름
  → {"decision":"block","reason":"..."} JSON stdout 출력, 상태 파일 갱신
...
- **무한 반복 발화 (실측 발견 후 수정됨)**: 추적 안 된 파일이 커밋되지 않고 계속 방치되면, 상태 추적 없이는 매 Stop마다 동일한 지시가 반복 발화됨(실제 라이브 검증에서 확인). `HEAD SHA + 변경 경로`를 `.git/team-progress-last-state`에 기록해 해결 — 커밋되거나 실제로 더 바뀌기 전까지는 조용
...
3. ✅ 같은 상태(추적 안 된 파일 방치)로 재발화하지 않음, 커밋 후 재수정 시에는 다시 발화함을 `test_check_team_progress.py`의 상태 추적 테스트 3건으로 확인
4. `wiki-lock.sh` 동시 pane 시뮬레이션은 기존대로 미실시(vault 자체 테스트로 커버 — 검증 방법 섹션의 원래 근거 유지)
