    ...
    "Stop": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cd \"$PWD\" && git diff --name-only HEAD 2>/dev/null | grep -qE '^(docs/|memory_store/|cloud-builder/|todo\\.py|test_)' && echo 'TEAM_PROGRESS_CHANGED: 이번 세션에서 산출물이 바뀌었습니다. 지금까지의 대화 맥락에서 네가 어느 팀원(쭌/민준/지훈/수아/서연/태양)인지 판단한 뒤, ~/workspaces/multi-agent-wiki 디렉토리로 가서 bash scripts/wiki-lock.sh acquire wiki/team/<이름>.md 로 락을 잡고, wiki/team/<이름>.md 파일의 \"## 최근 작업\" 섹션에 이번 세션에서 한 일을 3~5문장으로 요약해 추가(append, 기존 내용 유지)한 뒤, git add 및 git commit을 실행하고, bash scripts/wiki-lock.sh release wiki/team/<이름>.md 로 락을 해제하라.' || true"
          }
        ]
      }
    ]
...
command-type을 쓰는 이유: prompt-type Stop 훅은 알려진 "Plugin Hook STDOUT 버그"(`anthropics/claude-code#10875`)로 컨텍스트 주입이 실패할 수 있음 — claude-obsidian 자체가 이 이유로 command-type을 표준으로 채택했고 여기서도 동일하게 따름.
...
민준 pane에서 응답 완료 (Stop 이벤트)
  ...
  → docs/architecture.md가 이번 응답에서 바뀜 → TEAM_PROGRESS_CHANGED 텍스트 stdout 출력
  → Claude Code가 이 텍스트를 다음 컨텍스트로 받음
  → 민준 페르소나가 "나는 민준이다" 판단
