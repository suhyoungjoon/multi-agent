...
> 상태: 승인됨, 실제 tmux 라이브 검증 완료 + 최종 리뷰 수정 반영 (2026-07-14/15)
> - 라이브 검증: `rm -rf` 실제 차단 확인, Track B Stop 훅과 정상 공존(같은 세션에서 동시 발화), wiki-lock.sh 실제 경합 상황 정상 처리
> - 최종 리뷰에서 두 가지 발견: (1) `rm -Rf`/`--recursive --force`/`-r --force` 등 대소문자·long-flag 조합이 원래 정규식을 우회 — "옵션 순서/결합 무관하게" 보장이 실제로는 깨져 있었음 → 두 개의 독립 조건(recursive 존재 AND force 존재, 소문자 정규화)으로 재작성해 해결. (2) 알림 문자열에 `$COMMAND`를 AppleScript 소스에 직접 삽입해서, 따옴표가 포함된 명령이 AppleScript 인젝션으로 실제 임의 명령을 실행할 수 있었음(라이브로 `do shell script` 실행까지 확인) → `on run argv` 방식으로 명령을 데이터 인자로만 전달하도록 수정, 재검증으로 차단 확인
> - Minor 3건(`.env.example` 오탐, `main-*` 브랜치명 오탐, `HEAD:main` refspec 우회)은 선례 스크립트에서 물려받은 기존 특성으로 이번 라운드에서는 수정하지 않음(낮은 우선순위, 알려진 한계로 기록)
...
## 알려진 한계 (최종 리뷰에서 발견, 낮은 우선순위로 미수정)

- **`.env.example`/`.env.sample` 오탐**: 시크릿이 없는 템플릿 파일도 `.env*` 패턴에 걸려 차단됨(예: `cat .env.example`). `cp .env.example .env` 같은 정상 셋업 절차도 막힘
- **`main-*`/`master-*` 브랜치명 오탐**: `git push origin main-ui`가 `main` 브랜치 직접 push로 오인되어 차단됨(정규식이 `main`으로 시작하는 브랜치명 전체를 매칭)
- **`HEAD:main` refspec 우회**: `git push origin HEAD:main`처럼 브랜치 토큰이 `main`으로 시작하지 않는 refspec은 감지 못함 — 패턴3이 막으려는 상황을 정확히 우회 가능
- 세 항목 모두 선례 스크립트(`claude-code-understanding`)에서 그대로 물려받은 기존 특성이며, 이번 스코프에서 발생한 회귀가 아님. 실사용에서 문제가 되면 재검토
