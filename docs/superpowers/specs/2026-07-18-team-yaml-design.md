...
- **watchdog.sh 위치 발견**: 계획 수립 중 `watchdog.sh`가 `multi-agent` 저장소 안이 아니라 `~/Downloads/watchdog.sh`에 있어 git 버전관리가 전혀 안 되고 있음을 발견. 이번 작업에서 저장소 루트로 이동시켜 다른 팀 관리 스크립트(`setup-team-v2.sh`)와 같은 위치에서 버전관리되도록 함
